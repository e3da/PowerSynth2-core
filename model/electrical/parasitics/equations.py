...
def self_mcpm_trace_imp(input_mat=[], iso_thick = 0.68, rs_model = None, frequency = 1e6):
    """_summary_

    Args:
        input_mat (list, optional): _description_. Defaults to [].
        iso_thick (float, optional): _description_. Defaults to 0.68.
        rs_model (_type_, optional): _description_. Defaults to None.
    ...
    for i in range(len(input_mat)):
        w,l,t = input_mat[i] # get the mesh info width, length, trace_thick in um (integer)
        if w < 500: # edge mesh
            R = trace_resistance(frequency,w/1e3,l/1e3,t/1e3)
...
def self_imp_py_mat(input_mat=[],f=1e8,type = 'trace',eval_type ='equation'):
    ...
    This is used for design without rs-model or layerstack. Or mainly 3D scenarios
    Args:
...
def unpack_and_eval_RL_Krigg(f=1e6,w =[],l=[],mdl=None):
    ...
    #print('f=',freqquencies[freq_id+1])
